        ...
        for file in files:
            if file.endswith('ckpt'):
                ...
                if deterministic_policy:
                    ...
                    turple.append(' ')
                if "walker2d-medium-replay-v2" in grandparent_dir:
                    if corrupt_dynamics:
...
rpex_commands = [
    [
        "python3", f"attack_online.py", f"--algorithm=rpex", f"--env_name={env}", f"{normalize_states}",
        f"--kappa={kappa}", f"--seed={seed}", f"{corruption_flag}", f"--log_dir=ablation_buffer/rpex", f"--ckpt_path={ckpt_path}",
        f"--corruption_range={ranges}", f"--corruption_rate={rate}"
    ] + (["--updates_per_step=4"] if "acts" in corruption_flag else [])
...
riql_pex_commands = [["python3", f"attack_online.py", f"--algorithm=riql", f"--env_name={env}",f"{normalize_states}", \
    f"--inv_temperature={inv}",f"--seed={seed}",f"{corruption_flag}",f"--log_dir=ablation_buffer/riql_pex",f"--ckpt_path={ckpt_path}", \
        f"--corruption_range={ranges}",f"--corruption_rate={rate}"] 
...
f"--seed={seed}",f"{corruption_flag}",f"--log_dir=ablation_buffer/riql_direct",f"--ckpt_path={ckpt_path}", \
        f"--corruption_range={ranges}",f"--corruption_rate={rate}"] 
    for seed in seeds  for env, ckpt_path,corruption_flag,normalize_states,_ in riql_attack for ranges,rate in corrupt_list]


# print(len(rpex_commands))
# print(rpex_commands)
# Combine all commands
commands =  rpex_commands + riql_pex_commands + riql_direct_commands

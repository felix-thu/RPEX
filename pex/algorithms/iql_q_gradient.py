...
class IQL_Q_Gradient(nn.Module):
    ...
    def policy_update(self, observations, adv, actions):
        ...
        action_samples = policy_out.sample()
        q1_new_action, q2_new_action = self.critic(observations, action_samples)
                    
        if np.random.uniform() > 0.5:
            policy_loss = - q1_new_action.mean() / q2_new_action.abs().mean().detach()
        else:
            policy_loss = - q2_new_action.mean() / q1_new_action.abs().mean().detach()
